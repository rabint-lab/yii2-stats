...
class stats extends \yii\base\Module
{
    ...
    public static function analiseStats($fromFirst = false)
    {
        ignore_user_abort();
        set_time_limit(300);
        ...
        $tsFrom = null;
        if (!$fromFirst) {
            $tsFrom = \rabint\stats\models\Stats::find()
                ->orderBy(['date' => SORT_DESC])
                ->select('date')
                ->scalar();
        }
        if ($fromFirst or (empty($tsFrom))) {
            $sql = <<<SQL
            SELECT (DATE( FROM_UNIXTIME( time ) )) as date FROM `stat_dailies` ORDER BY `time` ASC LIMIT 1
SQL;
            $tsFrom = \Yii::$app->db->createCommand($sql)->queryScalar();
        }

        $startDate = new \DateTime($tsFrom);
...
//            }
            ...
            \rabint\helpers\process::simpleOutputProgress($k + 1, $total, "done analyse date: " . $date);
            self::keyStorageSet('Stats.AnalyseAllStatus', 'doing-' . $date);
        }
        self::keyStorageSet('Stats.AnalyseAllStatus', 'done');
...
//
//        $model->group = $group;
//        $model->group_category = $group_category;
